...
def handler(event, context):
    ...
    q = ("INSERT INTO landsat_ac_log (path, row, acquisition, jobid) VALUES"
         + "(:path::varchar(3), :row::varchar(3), :acquisition::date, :jobid::text)"
         + " ON CONFLICT ON CONSTRAINT no_dupe_pathrowdate DO UPDATE SET jobid = excluded.jobid;")
    execute_statement(
        q,
        sql_parameters=[
            ...
            {"name": "jobid", "value": {"stringValue": "test"}},
        ],
    )
    return event
